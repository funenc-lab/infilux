...
Infilux 是基于此前以 **EnsoAI** 对外发布的项目继续二次开发而来的版本。

当前应用延续了 EnsoAI 既有代码库和产品方向，并在此基础上以 Infilux 名称继续演进，
包括仓库归属、运行时标识、文档和发布基础设施的更新。它不是从零重写的全新项目。
当前应以下面这些地址作为正式入口：
...
- 后续开发、打包发布和支持工作都以 Infilux 标识继续进行
...
- 当前代码库延续自同一条项目演进线
- 当前实现基于早期 EnsoAI 代码库，并在此基础上继续开发
- 既有发布历史与历史资料依然有效
...
> Infilux 是当前产品名。GitHub Releases 是当前正式且已文档化的发布源。
...
Homebrew、Scoop 和 Winget 当前不作为 Infilux 最新版本的文档化安装入口。
除非维护者明确宣布某个渠道已经针对特定版本重新验证，否则不要依赖旧的 `ensoai` 或 `J3n5en.EnsoAI` 包管理器标识。
...
Infilux 可以发现并控制投影到受支持 Agent 会话中的 skill、command、subagent 与 MCP server。

- 在设置中查看已发现的 Claude 能力、共享 MCP server 与个人 MCP server
- 配置全局基线、仓库策略和 worktree 级覆盖策略，控制 Skill 与 MCP 访问范围
- 在创建受支持 Agent 会话前打开单次会话启动选项
- 当运行中的会话需要重启才能应用新的 Skill 或 MCP 设置时显示 stale 提示

---

### Agent 会话画布与 Subagent
