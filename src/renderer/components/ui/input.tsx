...
function Input({ className, size = 'default', unstyled = false, ...props }: InputProps) {
  return (
    ...
    >
      <InputPrimitive
        className={cn(
          'h-8.5 w-full min-w-0 rounded-[inherit] px-[calc(--spacing(3)-1px)] leading-8.5 outline-none placeholder:text-muted-foreground/72 focus-visible:outline-none focus-visible:outline-offset-0 focus-visible:shadow-none sm:h-7.5 sm:leading-7.5',
          size === 'sm' &&
