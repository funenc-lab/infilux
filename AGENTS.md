...
- Use `Infilux` as the current app identity for packaging, runtime paths, and managed logs. The active runtime log path is the Infilux path returned by `app.getPath('logs')`, and managed log files use the `infilux-` prefix.
